...
for tr in detail_tr_list:
    ...
    td_list = [td_list[0], td_list[3], td_list[2], td_list[4]]
    single_data_list = []
    for td in td_list:
...
df[1] = df[1].map(str) + df[2].map(str)
del df[2]
df.columns = ['材料', '型号', '价格']
print(df)
df.to_excel('./每日明细数据/最新数据' + detail_data_date + '.xlsx', sheet_name='硅钢')
name_list = list(df['型号'])
...
def get_data_loc(name):
    for i_new in range(1, 100, 1):
        for j_new in range(1, 100, 1):
