...
for li in url_list:
    ...
    if next_title:
        if '热轧板卷' in next_title[0] and '（' not in next_title[0] and '(' not in next_title[0] and title_check in next_title[0]:
            print(next_title[0])
...
df['表格表头'] = table_title
print(df)
df.to_excel('./每日明细数据/最新数据' + detail_data_date + '.xlsx', sheet_name='热轧板卷', index=False)
